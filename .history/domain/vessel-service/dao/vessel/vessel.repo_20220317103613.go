...
func (repo *VesselPepo) Preload() (*gorm.DB, error) {
	result := GetVesselDB(context.Background(), repo.DB).Preload("Vessel")
	return result, errors.WithStack(result.Error)
}

func (repo *VesselPepo) Get(wherestr string) ([]*Vessel, error) {
	var cs []*Vessel
	result := GetVesselDB(context.Background(), repo.DB).Find(&cs).Where(wherestr)
	return cs, errors.WithStack(result.Error)
}
