...
func (repo *VesselPepo) QueryByCapacityMaxWeight(Capacity string, MaxWeight int32) ([]*Vessel, error) {
	var cs []*Vessel

	result := GetVesselDB(context.Background(), repo.DB)
	if len(Capacity) > 0 {
		result.Where("Capacity<= ? ", Capacity)
	}
	if MaxWeight > 0 {
		result.Where("MaxWeight <=?", MaxWeight)
	}
	result.Find(&cs)
	return cs, errors.WithStack(result.Error)
}
